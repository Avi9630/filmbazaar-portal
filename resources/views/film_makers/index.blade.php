    ...
    <div class="row mt-3">
        ...
        <div class="col-md-4">
            ...
            <select id="paymentStatus" name="paymentStatus" class="form-select" style="width: 100%">
                <option value="">All Status</option>
                <option value="1" {{ request('paymentStatus') == '1' ? 'selected' : '' }}>Paid</option>
                <option value="0" {{ request('paymentStatus') == '0' ? 'selected' : '' }}>Unpaid</option>
            </select>
        </div>
        <div class="col-md-4">
            <label for="statusdata">From Waves India </label>
            <select id="from_waves" name="from_waves" class="form-select" style="width: 100%">
                <option value="">All Status</option>
                <option value="1" {{ request('from_waves') == '1' ? 'selected' : '' }}>Yes</option>
                <option value="0" {{ request('from_waves') == '0' ? 'selected' : '' }}>From Waves Bazaar</option>
            </select>
        </div>
        <div class="col-md-4">
            <label for="statusdata">Agree For meeting </label>
            <select id="agree_for_meeting" name="agree_for_meeting" class="form-select" style="width: 100%">
                <option value="">All Status</option>
                <option value="1" {{ request('agree_for_meeting') == '1' ? 'selected' : '' }}>Yes</option>
                <option value="0" {{ request('agree_for_meeting') == '0' ? 'selected' : '' }}>No</option>
            </select>
        </div>
    </div>
                ...
                <div class="dropdown">
                    ...
                    <ul class="dropdown-menu">
                        <li><a class="dropdown-item" href="{{ route('film_makers.show', $filmMaker->id) }}">View
                                Details</a></li>
                        <!-- <li><button class="dropdown-item" id="convert" data-id="{{ $filmMaker->id }}">Convert into Buyer</button></li> -->
                        ...
                        @if ($filmMaker->payed === 1 && $filmMaker->asigned_b2b==0)
                        <li><button class="dropdown-item sendB2b" data-id="{{ $filmMaker->id }}">Send to
                                B2B</button></li>
                        @elseif($filmMaker->payed === 1 && $filmMaker->asigned_b2b==1)
                        <li><button class="dropdown-item sendB2b" data-id="{{ $filmMaker->id }}">Re-send to B2B</button></li>
                        @endif
                    </ul>
                </div>
...
<!-- Pagination Links -->
<div>
    {{ $filmMakers->appends(request()->query())->links() }}
    ...
    <script>
        $(document).ready(function() {
            ...
            $(".deactivate").click(function(e) {
                ...
                $.ajax({
                    url: @json(route('film_makers.update.status')), // Ensures proper formatting
    ...
    <script>
        $(document).ready(function() {
            $('.sendB2b').click(function() {
                ...
                $.ajax({
                    url: @json(route('send.b2b')), // Ensures proper formatting
